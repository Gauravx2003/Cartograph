...
export const RepoPicker = ({ onConfirm }: { onConfirm: (repo: string) => void }) => {
    ...
    <div className="w-full max-w-lg mx-auto flex flex-col items-center mt-6">
      <div className="w-full bg-canvas border border-hairline rounded-lg shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="flex-1 max-h-[300px] overflow-y-auto">
          {loading ? (
            <div className="p-4 text-center text-sm text-mute">Loading repositories...</div>
          ) : error ? (
            <div className="p-4 text-center text-sm text-risk-high">{error}</div>
          ) : filteredRepos.length === 0 ? (
            <div className="p-4 text-center text-sm text-mute">No repositories found.</div>
          ) : (
            <ul className="flex flex-col">
              {filteredRepos.map(repo => (
                <li key={repo.fullName}>
                  ...
                  >
                    <span className="font-mono text-sm text-ink">{repo.fullName}</span>
